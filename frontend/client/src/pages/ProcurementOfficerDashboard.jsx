...
export default function ProcurementOfficerDashboard() {
  ...
  const [actionType, setActionType] = useState('under_review');
  ...
  const markUnderReviewMutation = useMutation({
    mutationFn: (id) => api.markLoanUnderReview(id),
    onSuccess: () => {
  ...
  const approveLoanMutation = useMutation({
    mutationFn: (id) => api.approveLoan(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      setIsActionDialogOpen(false);
  ...
  const disburseLoanMutation = useMutation({
    mutationFn: (id) => api.disburseLoan(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      setIsActionDialogOpen(false);
  ...
  const releaseLoanMutation = useMutation({
    mutationFn: (id) => api.releaseLoan(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      setIsActionDialogOpen(false);
  ...
  const rejectLoanMutation = useMutation({
    mutationFn: (id) => api.rejectLoan(id, rejectionReason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"] });
      setIsActionDialogOpen(false);
      setRejectionReason("");
  ...
  const underReviewLoans = loansData.filter((l) => l.status === 'under_review');
  const approvedLoans = loansData.filter((l) => l.status === 'approved');
  const disbursedLoans = loansData.filter((l) => l.status === 'disbursed');
